...
router.post('/token', function(req, res, next) {
    ...
    var sql='select * from machine_apps where appid=? and appsecret=?';
    db.query(sql, [req.body.appid,req.body.appsecret], function(rows) {
        ...
        db.query(sql,function(rs){
            if(parseInt(rs[0].timestamp)<parseInt(rows[0].expireTime)){
                sql='update machine_apps set expireTime=UNIX_TIMESTAMP(DATE_ADD(now(),INTERVAL 30 MINUTE)) where id=?';
                ...
                sql='update machine_apps set token=?,expireTime=UNIX_TIMESTAMP(DATE_ADD(now(),INTERVAL 30 MINUTE)) where id=?';
