var mixstrategyEdit = {
    ...
    valid:function(type){
        ...
        $('select[name=strategyType]').each(function(index,elem) {
            ...
            if($(this).val()==0){
                common.ajaxSelect('strategyId_'+index,'/mixstrategy/data_rp');
            }
            if($(this).val()==2){
                common.ajaxSelect('strategyId_'+index,'/mixstrategy/data_cards');
            }
            if($(this).val()==3){
                common.ajaxSelect('strategyId_'+index,'/mixstrategy/data_point');
            }
    ...
    addNew:function(){
        ...
        //$('.trlist').eq(length).find('input[name=enabled]').prop('checked',true);
    ...
    submit:function(){
        ...
        $('form input,form textarea,form select').each(function(i,n){
            ...
            var val=$(this).val();
            if($.trim(name)!=''){
                if(name=='strategyType'||name=='strategyId'||name=='weight'/*||name=='enabled'*/){
            ...
            success:function(d) {
                ...
                if(d.errorCode==0){
                    common.alert('保存成功',function(d){
                        if(d==1){
                            location.href='/mixstrategy/lists';
                        }
                    });
                }else{
