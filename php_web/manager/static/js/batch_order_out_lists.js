...
var order = {
    ...
    createTable:function(){
        ...
        var config={
            ...
            "ajax":{
                url:'/batch/order_list_data/out',//请求数据地址
                type:'POST',//请求方式
            },
            "columns": [
        ...
        $('#orderTable tbody td .del').off('click').on("click",function(){
            ...
            common.confirm('确定删除吗？',function(r){
                if(r==1){
                    ...
                    $.post('/batch/order_delete/'+id+'/out',function(d){
                        common.unloading();
                        if(d.errorCode==0){
    ...
    showDetail:function(id){
        $('.info').remove();
        var html='<div id="detail" class="info" style="display:none"><div class="layer"></div><dl class="box"><dt>出库单详情</dt><dd><div class="condiv">';
        $.post('/batch/order_detail/'+id,function(data){
            ...
            s+= data.productName;
            s+= '</td><td class="name">销往客户编码：</td><td class="value">';
            s+= data.saleToCode;
            s+= '</td></tr>';
            s+= '<tr><td class="name">客户名称：</td><td class="value">';
            s+= data.saleToName;
            s+= '</td><td class="name">销往区域的编码：</td><td class="value">';
            s+= data.saleToAgc;
            s+= '</td></tr>';
            s+= '<tr><td class="name">出库时间：</td><td class="value">';
            s+= data.orderTime;
            s+= '</td>';
            s+= '<td class="name">上传时间：</td><td class="value">';
            s+= data.putTime;
            s+= '</td></tr>';
