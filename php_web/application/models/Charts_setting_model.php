 ...
 * 排行业务逻辑
 */
class Charts_setting_model extends CI_Model {
    ...
    // --------------------------------------------------------------------------
    function merchants() {
    	return $this->db->select('id,name,phoneNum')->get('merchants')->result();
    }
    // --------------------------------------------------------------------------
	// 根据城市名称查找数据库中对应的Code
	 ...
	 * @param 参数 $param
	 */
	function get_table_data($mch_id,$param,&$count=0,$start=null,$length=null){
		$time=get_time_screening($param);
		//全国数据排名 不区分省市区
		$sql="select m.userId,u.nickName,m.scanNum from (select a.userId,sum(a.scanNum) scanNum  from rpt_user_rank a
where a.mchId=? and a.theDate>=? and a.theDate<=? ";
		...
		//省份数据
		$sql2="select m.userId,u.nickName,m.scanNum from (select a.userId,sum(a.scanNum) scanNum  from rpt_user_rank a
where a.mchId=? and proCode=? and a.theDate>=? and a.theDate<=? ";
		$sql2_cnt="select count(m.id) cnt from (select id from rpt_user_rank where mchId=? and proCode=? and theDate>=? and theDate<=? ";
		//城市
		$sql3="select m.userId,u.nickName,m.scanNum from (select a.userId,sum(a.scanNum) scanNum  from rpt_user_rank a
where a.mchId=? and cityCode=? and a.theDate>=? and a.theDate<=? ";
		...
		if($param['pro']==0){//全国
            ...
            $sql.="group by a.userId order by scanNum desc,a.userId ";
            ...
            $sql2.="group by a.userId order by scanNum desc,a.userId ";
            ...
            $sql3.="group by a.userId order by scanNum desc,a.userId ";
